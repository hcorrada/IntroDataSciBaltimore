...
bpd <- read.csv("BPD_Arrests.csv", header=TRUE)

# Convert the ArrestDate column
bpd$ArrestDate <- as.Date(bpd$ArrestDate, "%m/%d/%Y")

# Remove old arrests and arrests without reported locations
bpd <- subset(bpd, ArrestDate>="2015-01-01")
bpd <- subset(bpd, bpd$Location.1 != "")
...
vacantBldgs <- subset(vacantBldgs, vacantBldgs, vacantBldgs$Location.1 != "")
...
balto_map = subset(map_data("county", region="maryland"),subregion=="baltimore
city")
plt=ggplot()
plt=plt+geom_polygon(data=balto_map,aes(x=long,y=lat),color="black",fill="gray40")
